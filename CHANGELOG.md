...
- **Automatic active handoff** - When another device is active, factory now automatically performs fullSync then setActive to claim active status. Eliminates "Result must be unique" merge errors by building ID mappings before attempting to become active.
...
- **Restore conflict resolution** - Added back `setActive()` call after bidirectional sync to resolve conflicting actives. Without this, wallet operations fail with `WERR_NOT_ACTIVE` when remote has different `activeStorage`.
...
- **Immediate broadcast sync** - Intercept `createAction` and `signAction` to sync to remote backup after immediate broadcasts (`acceptDelayedBroadcast: false`). Previously, only delayed broadcasts triggered the monitor callback which synced to backup.

## [0.0.66] - 2026-02-02

### Fixed

- **sendAllBsv** - Use `maxPossibleSatoshis` magic constant instead of manually listing inputs, fixing "unlockingScript required" error when sweeping many UTXOs

## [0.0.65] - 2026-02-02

### Fixed

- **Fallback to local-only mode** - If remote sync or conflict resolution fails, wallet now operates in local-only mode instead of being locked with `isActiveEnabled: false`

## [0.0.64] - 2026-02-02

### Added

- **Sync from remote on startup** - Wallet now automatically pulls changes from remote storage during initialization to sync with other devices
